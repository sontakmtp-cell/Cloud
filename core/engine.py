...
from .utils import deg2rad, parse_trough_label, capacity_from_geometry_tph
...
def get_default_spacings(width_mm: int, capacity_tph: float) -> Tuple[float, float]:
    ...
    return lc_table[closest_lc], lr_default
...
class CalculationStrategy(ABC):
    ...
    def _effective_drive_contact(self) -> Tuple[float, float, bool]:
        ...
        if dt == "tail drive":
            return max(base_wrap - 30.0, 120.0), base_mu * 0.95, False
        if dt == "center drive":
            return min(max(base_wrap, 160.0), 200.0), base_mu * 0.97, False
        # Dual drive is handled separately, but we return values for consistency
        if dt == "dual drive":
            return min(base_wrap + 20.0, 240.0), base_mu, True
        return base_wrap, base_mu, False
    ...
    def _apply_geo_limitation_to_load(self):
        ...
        self.r.mass_flow_rate = q_eff * V
        self.r.Qt_effective_tph = self.r.mass_flow_rate * 3.6 / 1000.0
        if q_eff < q_from_Qt - 1e-6:
            self.r.warnings.append(
                f"Lưu lượng thực bị khống chế bởi tiết diện: Q_thực≈{self.r.Qt_effective_tph:.1f} t/h < Q_yêu cầu={self.p.Qt_tph:.1f} t/h."
            )

    # --- [BẮT ĐẦU NÂNG CẤP] ---
    def _calculate_single_drive_tensions(self):
        """Tính toán lực căng cho hệ thống truyền động đơn."""
        self.r.effective_tension = self.r.required_power_kw * 1000.0 / max(self.p.V_mps, 0.1)
